...
def train(datakind):
    ...
    pretrained_weight = torch.load('./populations_100/models_100t/model_{}.pth'.format(datakind))
    ...
    for epoch in range(MAX_EPOCH):
        ...
        if val_acc > best_acc:  # 判断每次在验证集上的准确率是否为最大
            best_epoch = epoch
            best_acc = val_acc
            torch.save(model.state_dict(), "./model_fine_100/model_{}.pth".format(datakind))  # 保存验证集上最大的准确率
...
def test(datakind):
    ...
    modelpath = r"./model_fine_100/model_" + datakind + ".pth"
...
def eval(cur_order,task_num,order):
    ...
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=32, shuffle=True)
    from newmodel_t import NewModel
...
def main():
    ...
    # order = [11, 15, 16, 20, 23, 24, 27, 31, 32, 35, 40, 41, 44, 5, 50, 56, 6, 63, 66, 71, 74, 79, 82, 87, 88, 92, 93,
    #           95, 96]
    order = [5, 24, 92, 35, 63, 56, 82, 15, 6, 27, 23, 79, 31, 69, 49, 60, 89, 7, 93, 33, 12, 91, 9, 94, 47, 86, 74, 39, 13, 84,
     72, 70, 77, 83, 87, 17, 48, 68, 75, 90, 59, 19, 42, 21, 14, 76, 66, 57, 95, 54, 11, 98, 53, 38, 65, 37, 8, 40, 20,
     62, 4, 99, 16, 96, 61, 32, 78, 50, 29, 18, 2, 0, 58, 44, 30, 88, 3, 52, 73, 36, 43, 28, 1, 67, 10, 41, 22, 97, 55,
    ...
    plt.show()
    plt.savefig('./eval_acc_fine_60.png')
