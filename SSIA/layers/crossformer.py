...
from .focalloss import GatedFusion
...
class CrossFormerBlock(nn.Cell):
    ...
    def construct(self, x):
        ...
        x = self.attn(x, mask=self.attn_mask)  # nW*B, G*G, C 128 49 96 
...
class CrossFormer(nn.Cell):
    ...
    def __init__(self, img_size=224, patch_size=[4, 8, 16, 32], in_chans=3, num_classes=1000,
                 embed_dim=96, depths=[2, 2, 1], num_heads=[3, 6, 12, 24],
        ...
        self.c=nn.Dense(128,196)
        self.h=nn.Dense(75*16,384)
        self.ca=GumbelAttention()
    ...
    def construct_features(self, x, x1):
        """ get features """
        ...
        # aud=self.h(x1.view(x1.shape[0],x1.shape[1],-1)).transpose(-1,-2)
        # aud=self.c(aud).transpose(-1,-2)
        # x=self.ca(aud,x)
        # x1 shape torch.Size([60, 128, 75, 16])
        x1_reshaped = ops.Reshape()(x1, (x1.shape[0], x1.shape[1], -1))
        aud = self.h(x1_reshaped).transpose((0, 2, 1))
        aud = self.c(aud).transpose((0, 2, 1))
        x = self.ca(aud, x)
    ...
    def construct(self, x, x1):
        x = self.construct_features(x, x1)
        # x = self.head(x)
        # print(x.shape)
        x = self.ol(x)
        return x
...
class GumbelAttention(nn.Cell):
    def __init__(self):
        super(GumbelAttention, self).__init__()
        self.softmax = nn.Softmax(axis=-1)
        self.g = GatedFusion(384, 1)
        
    def construct(self, text, img):
        score = self.g(img, text)
        score = ops.ReduceMean()(score, 1) 
        img = score
        return img

if __name__ == '__main__':
    x1 = ms.ops.randn((1, 128, 75, 16))
