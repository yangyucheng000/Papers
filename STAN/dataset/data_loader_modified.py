...
class ReferDataset(ds.Dataset):
    SUPPORTED_DATASETS = {
        ...
        'reftext': {
            'splits': (
            'train', 'val', 'test', 'subtest_street', 'subtest_shelf', 'subtest_home', 'subtest_sport', 'subtest_other',
    ...
    def __init__(self, data_root, split_root='data', dataset='referit', imsize=256,
                 ...
                 split='train', max_query_len=128, lstm=False, bert_model='./bert_model/bert-base-uncased'):
        self.images = []
        ...
        self.split = split
        self.vocab = Vocab.from_file(bert_model + "/vocab.txt")
        self.tokenizer = BertTokenizer(vocab=self.vocab, do_lower_case=True)
        ...
        if self.lstm:
            ...
            param_dict = mindspore.load_checkpoint(corpus_path)  # 加载corpus.pth文件
            self.corpus = mindspore.load_param_into_net(self.corpus, param_dict)
        ...
        for split in splits:
            ...
            self.images += mindspore.load_checkpoint(imgset_path)
