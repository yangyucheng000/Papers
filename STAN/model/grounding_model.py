...
import mindspore
from cybertron import BertModel, BertConfig
import torch
...
class cross_attention_head(nn.Cell):
    def __init__(self, emb_size=256, tunebert=False, convlstm=False, bert_model='bert-base-uncased', leaky=False,
        ...
        # Text model
        myconfig = BertConfig(output_hidden_states=True)
        self.textmodel = BertModel.load('bert-base-uncased',config=myconfig)
    ...
    def construct(self, image, word_id, word_mask, word_st_position, bbox_st_list, word_id_st_sent, word_mask_st_sent,
                  st_list_bbox2word):
        # f = open("ms_construct.txt", 'w')
        # Visual Module
        # print("image type: ", type(image))
        batch_size = image.shape[0]
        # print("before visumodel", file=f)
        # print("image", image, file=f)
        raw_fvisu = self.visumodel(image)
        # print("raw_fvisu", raw_fvisu, file=f)
        # print("test here!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        if self.convlstm:
        ...
        # print("before first textmodel", file=f)
        # print("word_id_st_sent", word_id_st_sent, file=f)
        # self.textmodel.eval()
        text_output = self.textmodel(word_id_st_sent, token_type_ids=None, attention_mask=word_mask_st_sent)
        all_encoder_layers_st_sent = text_output[2][1:]
        ...
        # print("all_encoder_layers_st_sent", all_encoder_layers_st_sent, file=f)
        ...
        # print("before second textmodel", file=f)
        # print("word_id", word_id, file=f)
        # Language Module for expression
        text_word_id_output = self.textmodel(word_id,
                                               token_type_ids=None, attention_mask=word_mask)
        all_encoder_layers = text_word_id_output[2][1:]
        # print("all_encoder_layers", all_encoder_layers, file=f)
        raw_flang = (all_encoder_layers[-1][:, 0, :] + all_encoder_layers[-2][:, 0, :]
                     + all_encoder_layers[-3][:, 0, :] + all_encoder_layers[-4][:, 0, :]) / 4
        raw_fword = (all_encoder_layers[-1] + all_encoder_layers[-2] + all_encoder_layers[-3] + all_encoder_layers[-4]) / 4
        ...
        mask_word_att = ops.zeros_like(raw_fword)
        mask_st_att = ops.zeros_like(raw_fword_st_sent)
        for ii in range(batch_size):
            mask_word_att[ii, 1:len(ops.nonzero(word_mask[ii])) - 1, :] = 1
            mask_st_att[ii, 1:len(ops.nonzero(word_id_st_sent[ii])) - 1, :] = 1
        raw_fword_attn = raw_fword * mask_word_att
        raw_fword_st_sent = raw_fword_st_sent * mask_st_att
        # print("raw_fword", raw_fword, file=f)
        # print("mask_word_att", mask_word_att, file=f)
        # print("mask_st_att", mask_st_att, file=f)
        ...
        st_relevance_score = ops.Select()(st_relevance_score < THRES_PHI, ops.ZerosLike()(st_relevance_score),
                                          st_relevance_score)
        # print("st_relevance_score: ", st_relevance_score, file=f)
        # print("st_relevance_score: ", st_relevance_score.shape)
        weighted_st_feature_8x8 = st_grid_calculation(st_relevance_score, word_st_position, bbox_st_list,
                                                      ...
                                                      image.shape[2])
        # print("weighted_st_feature_8x8: ", weighted_st_feature_8x8, file=f)
        # print("raw_fvisu_8x8: ", raw_fvisu_8x8, file=f)
        ...
        # print("raw_fvisu_16x16: ", raw_fvisu_16x16, file=f)
        ...
        # print("before mapping_visu:", file=f)
        # print("raw_fvisu", raw_fvisu, file=f)
        fvisu = self.mapping_visu(raw_fvisu)
        # print("fvisu", fvisu, file=f)
        ...
        single_conf = self.txt_single_classifier(ops.Concat(axis=1)((max_feature_32x32, raw_fvisu))).view(batch_size,
                                                                                                          map_fvisu_orig_co.shape[
                                                                                                              1])
        # print("after txt_single_classifier: ", file=f)
        # print("single_conf: ", single_conf, file=f)
        single_conf_16 = self.txt_single_classifier(ops.Concat(axis=1)((max_feature_16x16, raw_fvisu_16x16))).view(
            batch_size, 16*16)
        # print("single_conf_16: ", single_conf_16, file=f)
        ...
        # print("single_conf_8", single_conf_8, file=f)
        ...
        # print("out_feat", out_feat, file=f)
        # print("cosine_weights", cosine_weights, file=f)
        # f.close()
