...
class RNNDecoderBase(nn.Cell):
    ...
    def construct(self, tgt, memory_bank, state, memory_lengths=None,
                step=None):
        """
        Args:
        ...
        if type(decoder_outputs) == list:
            decoder_outputs = ops.stack(decoder_outputs)

            for k in attns:
                if type(attns[k]) == list:
                    attns[k] = ops.stack(attns[k])
    ...
    def init_decoder_state(self, src, memory_bank, encoder_final,
        ...
        def _fix_enc_hidden(hidden):
            ...
            if self.bidirectional_encoder:
                ...
                #                         hidden[2:3]], 2)
                # else:
                hidden = ops.cat([hidden[0:hidden.shape[0]:2],
                                    hidden[1:hidden.shape[0]:2]], 2)
            # else:
...
class StdRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, state, memory_lengths=None):
        ...
        # Check
        tgt_len, tgt_batch, _ = tgt.shape
        output_len, output_batch, _ = rnn_output.shape
        ...
        decoder_outputs, p_attn = self.attn(
            rnn_output.swapaxes(0, 1),
            memory_bank.swapaxes(0, 1),
        ...
        if self.context_gate is not None:
            decoder_outputs = self.context_gate(
                emb.view(-1, emb.shape[2]),
                rnn_output.view(-1, rnn_output.shape[2]),
                decoder_outputs.view(-1, decoder_outputs.shape[2])
            )
            decoder_outputs = \
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, state, memory_lengths=None):
        ...
        input_feed_batch, _ = input_feed.shape
        _, tgt_batch, _ = tgt.shape
        ...
        for _, emb_t in enumerate(emb.split(1)):
            ...
            decoder_input = ops.cat([emb_t, input_feed], 1)
...
class RNNDecoderState(DecoderState):
    ...
    def __init__(self, hidden_size, rnnstate):
        ...
        self.input_feed = ms.Tensor(np.zeros(h_size), dtype=self.hidden[0].dtype).unsqueeze(0)
