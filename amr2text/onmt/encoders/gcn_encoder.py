...
class GCNEncoder(EncoderBase):
    ...
    def construct(self, src, lengths=None):
        ...
        if self.rnn is not None:
            # print("enter self.rnn is not None!")
            packed_emb = emb
            ...
            # print("enter self.rnn is None")
            memory_bank = emb

        memory_bank_gcn = self.gcn(memory_bank, adj.float())

        if self.rnn is not None:
            n = memory_bank_gcn.size(2) // 2
            h1 = memory_bank_gcn[-1].unsqueeze(0)
            batch_size = memory_bank_gcn.size(1)
            h = Tensor(np.zeros((2, batch_size, n)), dtype=mindspore.float32)
            h[0] = h1[:, :, 0:n]
            h[1] = h1[:, :, n:n * 2]
            encoder_final = (h, h)
            memory_bank = memory_bank_gcn
        else:
            h, _ = ops.max(memory_bank_gcn, 0)
            h = h.unsqueeze(0)
            encoder_final = (h, h)
            memory_bank = memory_bank_gcn
    ...
    def _initialize_bridge(self, rnn_type,
        ...
        self.bridge = nn.CellList([nn.Dense(self.total_hidden_dim,
    ...
    def _bridge(self, hidden):
        ...
        def bottle_hidden(linear, states):
            ...
            return ops.relu(result).view(size)
