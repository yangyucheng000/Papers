...
def get_fields(data_type, n_src_features, n_tgt_features):
    ...
    if data_type == 'text':
        ...
        return LOGICDataset.get_fields(n_src_features, n_tgt_features)
    else:
...
def get_num_features(data_type, corpus_file, side):
    ...
    if data_type == 'text':
        ...
        return LOGICDataset.get_num_features(corpus_file, side)
    else:
        raise ValueError("Data type not implemented")
...
def make_features(batch, side, data_type):
    ...
    if data_type == 'logic':
        if side == 'tgt':
            return ops.cat([level.unsqueeze(2) for level in levels], 2)
        return (ops.cat([level.unsqueeze(2) for level in levels], 2),
                adj,
                parents)
    elif data_type == 'text':
...
def build_dataset(fields, data_type, src_data_iter=None, src_path=None,
    ...
    def _make_examples_nfeats_tpl(data_type, src_data_iter, src_path, src_dir,
        ...
        if data_type == 'text':
            ...
            src_examples_iter, num_src_feats = \
                LOGICDataset.make_logic_examples_nfeats_tpl(
    ...
    if data_type == 'text':
        ...
        dataset = LOGICDataset(fields, src_examples_iter, tgt_examples_iter,
                              num_src_feats, num_tgt_feats,
                              src_seq_length=src_seq_length,
                              tgt_seq_length=tgt_seq_length,
                              dynamic_dict=dynamic_dict,
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    if data_type == 'text' or data_type == 'logic':
...
class msLOGICBatch:
    def __init__(self, batch_size, indices, src, src_parents, src_adj, src_map, tgt, alignment):
        self.batch_size = batch_size
        self.indices = Tensor(indices)
        self.src = (Tensor(src[0]), Tensor(src[1]))
        self.src_parents = src_parents
        self.src_adj = Tensor(src_adj)
        self.src_map = Tensor(src_map)
        self.tgt = Tensor(tgt)
        self.alignment = Tensor(alignment)
...
class LOGICBatch:
    ...
    def __init__(self, data, dataset, device, train):
        ...
        # print("print batch: ")
        # print_elements(self.dataset)
        self.indices = ms.Tensor(np.zeros(self.batch_size), dtype=ms.int64)
        for i in range(self.batch_size):
            if train:
                self.indices[i] = LOGICBatch.train_index
                LOGICBatch.train_index += 1
            else:
                self.indices[i] = LOGICBatch.valid_index
                LOGICBatch.valid_index += 1
        ...
        supports = []
        for example in data:
            # print("src, src_map, tgt")
            # print(example.src)
            # print(example.src_map)
            # print(example.tgt)
            size = len(example.src)

            size_matrix = example.src_graph.matrix.size()
            # print("matrix: ", example.src_graph.matrix)
            # print("size: ", size_matrix)
            src_lengths.append(size)
            # src_lengths.append(size_matrix[1])
            tree = read_tree(example.src_graph.parents_or_edges)
            adj = ms.Tensor(np.zeros((size_matrix[0], src_length, src_length)), dtype=ms.int64)
            adj[:, 0 : size_matrix[1], 0 : size_matrix[1]] = ms.Tensor(example.src_graph.matrix.numpy())
            # print(adj[1])
            src_identifiers = [str_stoi["<blank>"]] * (src_length)
            for i, item in enumerate(example.src):
                src_identifiers[i] = str_stoi[item]
            example_src = (tree, adj, ms.Tensor(np.asarray(src_identifiers), dtype=ms.int64))
            srcs.append(example_src[2])
            supports.append((example_src[0], example_src[1]))            
            if hasattr(example, 'src_map'):
                src_map = ms.Tensor(np.zeros((src_length, src_length + 2)), dtype=ms.float32)
                for i in range(example.src_map.shape[0]):
                    # print("shape", example.src_map[i].size())
                    # print(example.src_map[i])
                    src_map[i][example.src_map[i].item()] = 1
        ...
        if 'tgt' in dataset.fields:
            for example in data:     
                ...
                example_tgt = ms.Tensor(np.asarray(tgt_identifiers), dtype=ms.int64)
                ...
                if getattr(example, "alignment", None) is not None:
                    ...
                    example_alignment = ms.Tensor(np.asarray(align_identifiers), dtype=ms.int64)
                    alignments.append(example_alignment)

        lengths = ms.Tensor(np.asarray(src_lengths), dtype=ms.int64)
        ...
        for t, a in supports:
            trees.append(t)
            adjs.append(a)
        self.src = (ops.stack(srcs, 1), lengths)
        self.src_parents = trees
        self.src_adj = ops.stack(adjs, 1)
        # self.tgt = torch.stack(tgts, 1)
        
        if srcmaps != []:
            self.src_map = ops.stack(srcmaps, 1)
        if tgts != []:
            self.tgt = ops.stack(tgts, 1)
            if srcmaps != []:
                self.alignment = ops.stack(alignments, 1)
...
def ms_build_dataset_iter(opt, is_train=True):
    if is_train:
        data_file = opt.data + "." + "ms_train_data.pkl"
    else:
        data_file = opt.data + "." + "ms_val_data.pkl"
    with open(data_file, 'rb') as f:
        ms_batches = pickle.load(f)
    dataset_iter = []
    for batch in ms_batches:
        dataset_iter.append(
                msLOGICBatch(batch.batch_size, batch.indices, batch.src,
                             batch.src_parents, batch.src_adj, batch.src_map,
                             batch.tgt, batch.alignment))
    corpus_type = "train" if is_train else "valid"
    logger.info('Loading %s dataset from %s, number of examples: %d' %
                (corpus_type, data_file, len(dataset_iter)))
    return dataset_iter
...
def lazily_load_dataset(corpus_type, opt):
    ...
    pts = sorted(glob.glob(opt.data + '.' + corpus_type + '.[0-9]*.pkl'))
    if pts:
        for pt in pts:
            yield _lazy_dataset_loader(pt, corpus_type)
    else:
        # Only one inputters.*Dataset, simple!
        pt = opt.data + '.' + corpus_type + '.pkl'
        yield _lazy_dataset_loader(pt, corpus_type)


class MyVocab:
    def __init__(self, freqs, itos, unk_index, stoi, vectors):
        self.freqs = freqs
        self.itos = itos
        self.unk_index = unk_index
        self.stoi = defaultdict(int)
        self.stoi.update(stoi)
...
class MyField:
    def __init__(self, src_vocab, tgt_vocab):
        self.field_dict = {
            "src": SrcTgt(src_vocab),
            "tgt": SrcTgt(tgt_vocab)
        }
...
def _load_fields(opt, checkpoint):
    if checkpoint is not None:
        ...
        with open(opt.data + '.vocab1.pkl', 'rb') as f:
            fields = pickle.load(f)
    # fields = dict([(k, f) for (k, f) in fields.items()
    #                if k in dataset.examples[0].__dict__])

    # # 创建Vocab对象
    # src_vocab = MyVocab(fields["src"].vocab.freqs, fields["src"].vocab.itos, fields["src"].vocab.unk_index, fields["src"].vocab.stoi, fields["src"].vocab.vectors)
    # tgt_vocab = MyVocab(fields["src"].vocab.freqs, fields["tgt"].vocab.itos, fields["tgt"].vocab.unk_index, fields["tgt"].vocab.stoi, fields["tgt"].vocab.vectors)
    #
    # # 创建Field对象
    # fields = MyField(src_vocab, tgt_vocab)
    # with open(opt.data + '.vocab1.pkl', 'wb') as f:
