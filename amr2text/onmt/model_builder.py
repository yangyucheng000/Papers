...
def build_encoder(opt, embeddings):
    ...
    """
    print("encoder_type: ", opt.encoder_type)
...
def load_test_model(opt, dummy_opt, model_path=None):
    ...
    checkpoint = pickle.load(model_path, map_location=lambda storage, loc: storage)
...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    # Build encoder.
    print("encoder: ", model_opt.model_type)
    ...
    if not model_opt.copy_attn:
        ...
        if model_opt.generator_function == "sparsemax":
            ...
            gen_func = nn.LogSoftmax(axis=-1)
        generator = nn.SequentialCell(
            nn.Dense(model_opt.dec_rnn_size, len(fields["tgt"].vocab)),
            gen_func
        )
    ...
    if checkpoint is not None:
        ...
        if model_opt.param_init != 0.0:
            for p in model.get_parameters():
                # print("p: ", p)
                p.set_data(ops.uniform(p.shape, Tensor(-model_opt.param_init, mindspore.float32), Tensor(model_opt.param_init, mindspore.float32)))
    ...
    # model.to(device)
    param_dict = mindspore.load_checkpoint("./data/logic_graph/ms_amr2text.ckpt")
