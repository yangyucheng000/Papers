...
class CopyGenerator(nn.Cell):
    ...
    * :math:`p_{copy}` the probility of copying a particular word.
    ...
    def __init__(self, input_size, tgt_dict):
        ...
        self.linear = nn.Dense(input_size, len(tgt_dict))
        self.linear_copy = nn.Dense(input_size, 1)
        self.tgt_dict = tgt_dict
        self.softmax = nn.Softmax(axis=1)
        self.sigmoid = nn.Sigmoid()

    def construct(self, hidden, attn, src_map):
        """
        ...
        out_prob = ops.mul(prob, 1 - p_copy.expand_as(prob))
        mul_attn = ops.mul(attn, p_copy.expand_as(attn))
        copy_prob = ops.bmm(mul_attn.view(-1, batch, slen)
                              .transpose(0, 1),
                              src_map.transpose(0, 1)).transpose(0, 1)
        copy_prob = copy_prob.contiguous().view(-1, cvocab)
        return ops.cat([out_prob, copy_prob], 1)
    ...
    def _compute_loss(self, batch, output, target, copy_attn, align):
        ...
        if self.data_type == 'text':
            scores_data = inputters.TextDataset.collapse_copy_scores(
                ...
                scores_data = inputters.LOGICDataset.collapse_copy_scores(
                self._unbottle(scores_data, batch.batch_size),
                batch, self.tgt_vocab, batch.dataset.src_vocabs)
        ...
        if self.normalize_by_length:
            ...
            loss = ops.div(loss, tgt_lens).sum()
        else:
            loss = loss.sum()
