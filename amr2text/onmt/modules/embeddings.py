...
class PositionalEncoding(nn.Cell):
    ...
    def __init__(self, dropout, dim, max_len=5000):
        pe = ops.zeros((max_len, dim), dtype=ms.float32)
        position = ops.arange(0, max_len).unsqueeze(1)
        div_term = ops.exp((ops.arange(0, dim, 2, dtype=ms.float32) *
    ...
    def construct(self, emb, step=None):
...
class Embeddings(nn.Cell):
    ...
    def __init__(self, word_vec_size,
        ...
        self.make_embedding = nn.SequentialCell()
        self.make_embedding.append(emb_luts)
        ...
        if feat_merge == 'mlp' and len(feat_vocab_sizes) > 0:
            ...
            mlp = nn.SequentialCell(nn.Dense(in_dim, out_dim), nn.ReLU())
            self.make_embedding.append(mlp)

        self.position_encoding = position_encoding

        if self.position_encoding:
            pe = PositionalEncoding(dropout, self.embedding_size)
            self.make_embedding.append(pe)
    ...
    def construct(self, source, step=None):
        """
        ...
        if self.position_encoding:
            for i, module in enumerate(self.make_embedding.cells()):
                if i == len(self.make_embedding.cells()) - 1:
        ...
        if self.emb_type == 'gcn':
            ...
            emb_treelstm = ops.stack(emb_treelstm, 1)
            ...
            for i in range(len(trees)):
                ...
                emb_treelstm_batch = ops.cat([emb_treelstm_batch, emb_treelstm_batch_down], 1)
                emb_treelstm.append(emb_treelstm_batch)         
            emb_treelstm = ops.stack(emb_treelstm, 1)
            # source = emb_treelstm
            source = ops.cat((source, emb_treelstm), 2)
            ...
            source = ops.cat((source, emb_gcn), 2)
            source = ops.cat((source, emb_treelstm), 2)
