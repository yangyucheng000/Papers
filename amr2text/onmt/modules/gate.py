...
class ContextGate(nn.Cell):
    ...
    def __init__(self, embeddings_size, decoder_size,
        ...
        self.gate = nn.Dense(input_size, output_size, bias=True)
        self.sig = nn.Sigmoid()
        self.source_proj = nn.Dense(attention_size, output_size)
        self.target_proj = nn.Dense(embeddings_size + decoder_size,
                                     output_size)

    def forward(self, prev_emb, dec_state, attn_state):
        input_tensor = ops.cat((prev_emb, dec_state, attn_state), axis=1)
        z = self.sig(self.gate(input_tensor))
        proj_source = self.source_proj(attn_state)
        proj_target = self.target_proj(
            ops.cat((prev_emb, dec_state), axis=1))
        return z, proj_source, proj_target


class SourceContextGate(nn.Cell):
