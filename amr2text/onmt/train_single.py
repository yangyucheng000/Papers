...
from onmt.inputters.inputter import build_dataset_iter, lazily_load_dataset, \
    _load_fields, _collect_report_features, ms_build_dataset_iter
...
def _tally_parameters(model):
    n_params = sum([p.asnumpy().size for p in model.get_parameters()])
    enc = 0
    dec = 0
    for param in model.get_parameters():
        if 'encoder' in param.name:
            enc += param.asnumpy().size
        elif 'decoder' or 'generator' in param.name:
            dec += param.asnumpy().size
...
def main(opt, device_id):
    ...
    def train_iter_fct():
        return ms_build_dataset_iter(opt)

    def valid_iter_fct():
        return ms_build_dataset_iter(opt, is_train=False)
