...
from onmt.inputters.inputter import msLOGICBatch
...
class Trainer(object):
    ...
    def train(self, train_iter_fct, valid_iter_fct, train_steps, valid_steps):
        ...
        # for i, batch in enumerate(train_iter):
        #     print(i, batch)
        #     ms_train_data.append(
        #         msLOGICBatch(batch.batch_size, batch.indices, (batch.src[0].asnumpy(), batch.src[1].asnumpy()),
        #                      batch.src_parents, batch.src_adj.asnumpy(), batch.src_map.asnumpy(),
        #                      batch.tgt.asnumpy(), batch.alignment.asnumpy()))
        #     if i == 281:
        #         break
        # with open("logic.ms_train_data.pkl", 'wb') as f:
        #     pickle.dump(ms_train_data, f)
        ...
        #     if i == 31:
        #         break
        # with open("logic.ms_val_data.pkl", 'wb') as f:
        #     pickle.dump(ms_val_data, f)
        # exit(0)
        total_stats = onmt.utils.Statistics()
        report_stats = onmt.utils.Statistics()
        self._start_report_manager(start_time=total_stats.start_time)
        print("self n_gpu: ", self.n_gpu)
        print("self.gpu_verbose_level: ", self.gpu_verbose_level)
                    ...
                    if accum == self.grad_accum_count:
                        ...
                        if self.n_gpu > 1:
                            normalization = sum(onmt.utils.distributed
                                                .all_gather_list
                                                (normalization))
                        # print("len(true_batchs): ", len(true_batchs))
    ...
    def validate(self, valid_iter):
        ...
        for batch in valid_iter:
            ...
            if self.data_type == 'text' or self.data_type == 'logic':
    ...
    def _gradient_accumulation(self, true_batchs, normalization, total_stats,
        ...
        for batch in true_batchs:
            ...
            for j in range(0, target_size - 1, trunc_size):
                ...
                batch_stats, grads = self.train_loss.sharded_compute_loss(
                    self.model, self.optim, batch, outputs, attns, j,
