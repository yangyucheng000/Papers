...
def process_dataset(dataset='AG', STOPWORD=False):
    ...
    if dataset == 'BNC':    
        ...
        data.to_csv(BNC_ADDR + '/raw/data.csv', quoting=1, header=True, index=False)
        ...
        N = len(data)
        SN = 9000    # valid + test = 9000
        ids = np.random.choice(range(N), size=SN, replace=False, p=None)
        N2 = len(ids)
        ids2 = np.random.choice(range(N2), size=int(SN / 3), replace=False, p=None)    # 3000 valid
        ids2 = np.array(ids)[ids2]
        data['train'] = 1
        data.iloc[ids, -1] = 0    # test split
        data.iloc[ids2, -1] = -1  # valid split

        data.to_csv(TMN_ADDR + '/raw/data.csv', quoting=1, header=True, index=False)
        ...
        data.to_csv(Reuters_ADDR + '/raw/data.csv', quoting=1, header=True, index=False)
...
if __name__ == '__main__':
    #==================================  20news =======================================
    # data, vocab = process_dataset(dataset='News20', STOPWORD=True)
    # print(len(data))
    # print(len(vocab))
    # data.to_csv(NEWS20_ADDR + '/overall_stop.csv', header=True, index=False, quoting=1)
    # clean_vocab(NEWS20_ADDR, freq_threshold=20, STOPWORD=True)
    # select_embedding(NEWS20_ADDR, STOPWORD=True)
    # eos_unk_pad_emd()
    #==================================  Reuters ======================================
    data, vocab = process_dataset(dataset='Reuters', STOPWORD=True)
    print(len(data))
    print(len(vocab))
    data.to_csv(Reuters_ADDR + '/overall_stop.csv', header=True, index=False, quoting=1)
    clean_vocab(Reuters_ADDR, freq_threshold=10, STOPWORD=True)
    select_embedding(Reuters_ADDR, STOPWORD=True)
    # eos_unk_pad_emd()
    #==================================  Tag My News ====================================
    ...
    # data, vocab = process_dataset(dataset='BNC', STOPWORD=True)
    # print(len(data))
    # print(len(vocab))
    # data.to_csv(BNC_ADDR + '/overall_stop.csv', header=True, index=False, quoting=1)
    # clean_vocab(BNC_ADDR, freq_threshold=120, STOPWORD=True)
    # select_embedding(BNC_ADDR, STOPWORD=True)
    # eos_unk_pad_emd()
    '''
    word freq threshold
    20 for 20NS, 5 for TMN, 120 for BNC, and 10 for Reuters

    edge freq threshold
    10 for 20NG, 3 for TMN, 40 for BNC, and 15 for Reuters
    '''
