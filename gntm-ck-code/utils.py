...
def print_top_words(beta, feature_names, n_top_words=20, common_texts=None):
    ...
    for i in range(len(beta)):
        line = " ".join([feature_names[j] for j in beta_idx[i, :n_top_words]])
    ...
    if common_texts:  
        ...
        for top_n in [5, 10, 15, 20, 25]:
            ...
            all_tc_results.append(results)
        all_tc_results = np.array(all_tc_results).transpose()
...
def eval_topic(beta, feature_names, n_top_words=20, common_texts=None, candidate_wordid=None):
    ...
    all_tc_results = print_top_words(beta, feature_names, n_top_words, common_texts)  
    all_td_results = []
    for n_word in [5, 10, 15, 20, 25]:
        td = get_topic_diversity(beta, n_top_words=n_word)
        all_td_results.append(td)

    return all_tc_results, all_td_results
